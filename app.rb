...
post '/refbot' do
  ...
  if input[0].downcase == "email" and eval(redis.hmget(params[:user_id], "candidate")[0])[:name].to_s != ""
    ...
    postback "New email for " + eval(redis.hmget(params[:user_id], "candidate")[0])[:name].to_s + ": " + eval(redis.hmget(params[:user_id], "candidate")[0])[:email].to_s + ". Type '/refbot phone <candidate phone>' to add a phone number. Step 3/5.", params[:channel_id], params[:user_name]
