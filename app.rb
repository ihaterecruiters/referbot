...
candidate_content = redis.hmget(params[:user_id], "candidate_0")
  ...
  if input[0].downcase == "name" and eval(candidate_content[0])[:name].to_s == ""
    redis.mapped_hmset(params[:user_id], {"candidate_0": {name: input[1..-1].join(" ").to_s, email: "", phone: "", vacancy: ""}, "step": "2"})
    # redis.mapped_hmset(params[:user_id], {"candidate_0": {name: input[1..-1].map { |x| x.to_s }.join(" ").to_s, email: "", phone: "", vacancy: ""}, "step": "2"})
