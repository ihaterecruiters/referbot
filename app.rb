...
post '/refbot' do
  ...
  when 'hello'
    priv_postback "Hello " + params[:channel], params[:channel_id], params[:user_name]
    break
  when 'list'
    getlist
    break
...
def priv_postback message, channel, user
    atuser = "@" + user
    slack_webhook = ENV['SLACK_WEBHOOK_URL']
    HTTParty.post slack_webhook, body: {"text" => message, "username" => "refbot", "channel" => "D1LK04JTW" }.to_json, headers: {'content-type' => 'application/json'}
end
